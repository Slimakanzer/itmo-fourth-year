...
typedef struct {
    ...
    XMylog2_uio_map maps[ MAX_UIO_MAPS ];
} XMylog2_uio_info;

/***************** Variable Definitions **************************************/
static XMylog2_uio_info uio_info;
...
static int uio_info_read_name(XMylog2_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XMylog2_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XMylog2_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XMylog2_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XMylog2_Initialize(XMylog2 *InstancePtr, const char* InstanceName) {
	XMylog2_uio_info *InfoPtr = &uio_info;
